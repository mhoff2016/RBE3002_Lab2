...
class Robot:
    ...
    def rotate(self, angle):
        ...
        startYaw = copy.deepcopy(self.yaw)
        final_yaw = startYaw + angle
        currentYaw = copy.deepcopy(self.yaw)
        angle_fix = angle + 2 * math.pi
        clockwise = .1
        counterclockwise = -.1
        if angle > 0:
            if startYaw > angle:
                while currentYaw > angle:
                    self.pubVel.publish(counterclockwise)
                    currentYaw = self.yaw 
            if startYaw < angle:
                while currentYaw < angle:
                    self.pubVel.publish(clockwise)

        if angle < 0:
            if startYaw > angle_fix:
                while currentYaw > angle_fix:
                    self.pubVel.publish(clockwise)
                    currentYaw = self.yaw
            if startYaw < angle_fix:
                while currentYaw < angle_fix:
